...
: 6Y@  ( -- y )  asm-label: emit6y c@ ; zx-inline
: 6X@  ( -- y )  asm-label: emit6x c@ ; zx-inline
...
|: (.CUR0)
  ...
  0<> DUP TOVER TINV
  BL (.CCX) ;
[ENDIF]  \ OPT-EMIT4-KCUR?


|: (CR?)
  6Y@ 6X@ #COLS - 0IF 0 6X! 1+ ENDIF
  DUP 23 U> IF DROP (SCROLL) 23 ENDIF 6Y! ; zx-inline

|: (.CR)  0 6X! 6Y@ 1+ 6Y! (CR?) ; zx-inline

|: (.>>)  6X@ 1+ #COLS MIN 6X! ; zx-inline

|: (.XX)  (CR?) (EMIT-RAW) (.>>) [ zx-has-word? OUT ] [IF] OUT 1+! [ENDIF] ; zx-inline
...
|: (KEY-NC)  ( -- code )
  ...
  [ENDIF]
  0 BEGIN DROP
    [ OPT-EMIT4-KCUR? ] [IF] .CUR [ENDIF]
    INKEY? TR-KEY
  DUP UNTIL KEY-BEEP
  [ OPT-EMIT4-KCUR? ] [IF] .CUR0 [ENDIF]
;
